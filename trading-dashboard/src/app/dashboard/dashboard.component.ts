...
export class DashboardComponent implements AfterViewInit {
  // @ViewChild(BaseChartDirective) chart?: BaseChartDirective; // Temporarily disabled
...
chooseUnmatchedTrade(tradeNotion: Trades, row: Table, rowIndex: number) {
  ...
  generateTradingChartData(): void {
    ...
    // Update chart data with beautiful animations
    this.chartData = {
      labels: labels,
      datasets: [
        {
          ...
          fill: true,
          tension: 0.4,
          pointBackgroundColor: balanceData.map((_, i) =>
            i === balanceData.length - 1 ? 'rgb(34, 197, 94)' : 'rgb(16, 185, 129)'
          ),
          pointBorderColor: '#ffffff',
          pointBorderWidth: 2,
          pointRadius: balanceData.map((_, i) => i === balanceData.length - 1 ? 8 : 5),
          pointHoverRadius: 10
        },
        {
          label: 'Cumulative P&L',
